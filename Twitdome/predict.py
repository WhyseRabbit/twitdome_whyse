...
def predict_user(user1, user2, tweet_text):
    """Predicts which user is most likely to say what."""
    user1 = User.query.filter(User.username == user1).one()
    user2 = User.query.filter(User.username == user2).one()
    user1_embed = np.array([tweet.embedding for tweet in user1.tweets])
    user2_embed = np.array([tweet.embedding for tweet in user2.tweets])
